...
@section('content')
    <div class="full-height flex-center" style="background-image: url({{ Settings::background() }})">
        <div class="central-form">
            <h1 class="center m-b-50">@{{ actionText }}</h1>
            <ajax-form action="{{ route('users.store') }}" alert-key="registration" :alert-duration="6000" method="POST"
                       redirect="{{ route('login') }}">
                <form-input v-if="userType == {{ config('starmee.user_type.athlete') }}" class="large"
                ...
                <form-input v-if="userType == {{ config('starmee.user_type.athlete') }}" class="large"
                            ...
                            :required="true"></form-input>
                <form-select v-if="userType == {{ config('starmee.user_type.athlete') }}" class="large" name="gender"
                             :required="true">
                    <option value="m">{{ trans('label.male') }}</option>
                    <option value="w">{{ trans('label.female') }}</option>
                </form-select>
                <form-input v-if="userType == {{ config('starmee.user_type.organizer') }}" class="large" name="name"
                            :max-length="{{ config('validation.organizer.name.max') }}"
                            :required="true"></form-input>
                <form-input class="large" type="email" name="email" :required="true"
                            :max-length="{{ config('validation.user.email.max') }}"></form-input>
                <form-input class="large" type="password" name="password" :required="true"
                            :max-length="{{ config('validation.user.password.max') }}"
                            :min-length="{{ config('validation.user.password.min') }}"></form-input>
                <form-input class="large" type="password" name="password_confirmation" :confirmed="true"
                            :required="true"></form-input>

                <form-switch class="center" name="user_type" :value="false"></form-switch>

                <div class="btn-group center">
                    <button type="submit" class="btn btn-large btn-primary">{{ trans('action.register') }}</button>
                </div>
